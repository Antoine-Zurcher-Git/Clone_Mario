...
void importImage() {
  ...
  Pblocs1 = loadImage("blocs/bH1.png");
}
...
void loadMap(int numCarte) {
  ...
  String cartep[] = loadStrings("Carte/"+numCarte+"p.txt");
  fond = "";
  for (int i = 5; i < cartep[0].length(); i ++) {
    fond += cartep[0].charAt(i);
  }
  passEtat(4);
}
  ...
  for (int i = cXbl; i < 49+cXbl+correctionX; i ++) {// block : 30*30
    for (int j = cYbl; j < 28+cYbl+correctionY; j ++) {
      if (j < carte.length-1) {
        ...
        if (nmB < Pblocs.length) {
          if(nmB  == 1 && j > 0 && (numBloc(carte[j-1].charAt(i)) == 0 || numBloc(carte[j-1].charAt(i)) == 13) ){
            image(Pblocs1, (i)*(tB), (j)*(tB), (tB), (tB));
          }else{
...
int numBloc(char bloc) {
  int a = 0;
  while (a < blocs.length && bloc != blocs[a] ) {
    a ++;
  }
  if (a == blocs.length) {
    a = ' ';
