...
//Menu
int etat = 0; //0 = Ecran d'acceuil ; 1 = Panneau de niveau ; 2 = Paramètre ; 3 = Credit ; 4 = Jeu ; 5 = Pause ; 6 = Victoire ; 7 = MapCreator ; 8 = choixMC
int paraRetour = 0;
...
void setup() {
  ...
  for (int i = 0; i < 7; i ++) {//x,y,lx,ly,Dtxt,etat,txt,corX,test,effet,color
    for (int j = 0; j < 4; j ++) {//rect(r(70+(220+40)*i+(220-txMenu)/2.0), r(80+(200+40)*j+(200-tyMenu)/2.0), r(txMenu), r(tyMenu), 7);
      ...
      boutonsNv[j*7+i] = new bouton(50+(220+40)*i+(220-txMenu)/2.0, 80+(200+40)*j+(200-tyMenu)/2.0, txMenu, tyMenu, 30, 1, str(j*7+i+1), -10, false, 4+j*7+i, cB);
    }
  }
...
void aff3() {
  ...
  //text("Antoine", r(mouseX), r(mouseY));
...
void aff4() {
  ...
  testFin();
  if (mousePressed) {
    //saveFrame("colltest/####.png");
  }
  tempsPartie += 1;//60.0/frameRate;
}
...
void aff5() {
  pushMatrix();
  //scale(1920.0/width);
  image(fondPara, 0, 0, 1920, 1080);//width, height);
  popMatrix();
}
...
void aff6() {
  ...
  for (int i = 0; i < nbSection; i ++) {
    ...
    float agl = i*2.0*PI/nbSection+tagl;
    float aglp1 = (i+1)*2.0*PI/nbSection+tagl;
    tagl += 0.0006;
    if (tagl > 2*PI) {
      tagl -= 2*PI;
    }
    float ox = 1920*2/3.0;
    float oy = 1080/2.0;
    triangle(ox, oy, ox+1920*cos(agl), oy+1920*sin(agl), ox+1920*cos(aglp1), oy+1920*sin(aglp1));
  }
  image(PiPerso, 1240, 475, 2*tB, 2*1.5*tB);
  //println(mouseX,mouseY);
  textSize(50);
  fill(0);
  text("Temps : "+round(100*tempsPartie/60.0)/100.0+" s", 400, 350);
  popMatrix();
}
